...
import { Copy, Check, FileText, Hash, ArrowRight } from 'lucide-react';
...
export const ExportActions: React.FC<ExportActionsProps> = ({ insight }) => {
  ...
  const copyToClipboard = async (type: 'notion' | 'obsidian') => {
    ...
    try {
      ...
      setCopyState(type);
      addToast(`${type === 'notion' ? 'Notion' : 'Obsidian'} formatted signal ready.`, 'success');
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 px-1 opacity-40">
         ...
         <span className="text-[7px] font-mono font-bold uppercase tracking-widest">Obsidian export includes YAML metadata.</span>
      </div>
    </div>
  );
};